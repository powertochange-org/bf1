    ...
    $.fn.makeEditable = function(options) {
        ...
        function _fnSetRowIDInAttribute(row, id, overwrite) {
            ...
            if (overwrite) {
                ...
                if (row.attr("id") == null || row.attr("id") == "") row.attr("id", id);
            }
        }
        ...
        function fnApplyEditable(aoNodes) {
            ...
            if (properties.bDisableEditing) return;
            var oDefaultEditableSettings = {
                ...
                "onsubmit": function(settings, original) {
                    ...
                    if (settings.type == "text" || settings.type == "select" || settings.type == "textarea") {
                        ...
                        if (input.length == 1) {
                            ...
                            if (oEditElement.nodeName.toLowerCase() == "select" || oEditElement.tagName.toLowerCase() == "select") sNewCellDisplayValue = $("option:selected", oEditElement).text(); //For select list use selected text instead of value for displaying in table
                            else sNewCellDisplayValue = sNewCellValue;
                        }

                        if (!properties.fnOnEditing(input, settings, original.revert, fnGetCellID(original))) return false;
                        var x = settings;
                        ...
                        if (settings.cssclass == null && settings.oValidationOptions == null) {
                            ...
                            if (!input.valid() || 0 == input.valid()) return false;
                            else return true;
                        }
                ...
                "submitdata": function(value, settings) {
                    ...
                    if (sColumnName == null || sColumnName == "") sColumnName = oTable.fnSettings().aoColumns[columnId].sTitle;
                    var updateData = null;
                    if (properties.aoColumns == null || properties.aoColumns[columnId] == null) {
                        ...
                        updateData = $.extend({}, properties.oUpdateParameters, properties.aoColumns[columnId].oUpdateParameters, {
                            "id": id,
                            "rowId": rowId,
                            "columnPosition": columnPosition,
                            "columnId": columnId,
                ...
                "callback": function(sValue, settings) {
                    ...
                    if (sValue.indexOf(properties.sFailureResponsePrefix) > -1) {
                        ...
                        if (properties.sSuccessResponse == "IGNORE" || (properties.aoColumns != null && properties.aoColumns[aPos[2]] != null && properties.aoColumns[aPos[2]].sSuccessResponse == "IGNORE") || (sNewCellValue == null) || (sNewCellValue == sValue) || properties.sSuccessResponse == sValue) {
            ...
            if (properties.aoColumns != null) {
                ...
                for (var iDTindex = 0, iDTEindex = 0; iDTindex < oSettings.aoColumns.length; iDTindex++) {
                    if (oSettings.aoColumns[iDTindex].bVisible) { //if DataTables column is visible
                        ...
                        try {
                            if (oColumnSettings.sUpdateURL != null) sUpdateURL = oColumnSettings.sUpdateURL;
                        } catch (ex) {}
...
/* Unblock KeyTable, but only after this 'esc' key event has finished. Otherwise
        ...
        function _fnOnRowDeleteInline(e) {
            ...
            if (sURL == null || sURL == "") sURL = properties.sDeleteURL;
        ...
        function fnDeleteRow(id, sDeleteURL) {
            ...
            var sURL = sDeleteURL;
            if (sDeleteURL == null) sURL = properties.sDeleteURL;
            properties.fnStartProcessingMode();
...
/* Function called after delete action
        ...
        */
        ...
        function fnGetActionSettings(sAction) {
            ...
            if (properties.aoTableAction) properties.fnShowError("Configuration error - aoTableAction setting are not set", sAction);
            var i = 0;

            for (i = 0; i < properties.aoTableActions.length; i++) {
                if (properties.aoTableActions[i].sAction == sAction) return properties.aoTableActions[i];
            }
        ...
        function fnPopulateFormWithRowCells(oForm, oTR) {
            ...
            $("input:text[rel],input:radio[rel][checked],input:hidden[rel],select[rel],textarea[rel],input:checkbox[rel]", $(oForm)).each(function() {
                var rel = $(this).attr("rel");

                if (rel >= iColumnCount) properties.fnShowError("In the form is placed input element with the name '" + $(this).attr("name") + "' with the 'rel' attribute that must be less than a column count - " + iColumnCount, "action");
                else {
                    ...
                    } else if (this.nodeName.toLowerCase() == "span" || this.tagName.toLowerCase() == "span") $(this).html(sCellValue);
                    else {
        ...
        function fnTakeRowDataFromFormElements(oForm) {
            ...
            $("input:text[rel],input:radio[rel][checked],input:hidden[rel],select[rel],textarea[rel],span.datafield[rel],input:checkbox[rel],input:password[rel]", oForm).each(function () {
                var rel = $(this).attr("rel");
                var sCellValue = "";
                if (rel >= iColumnCount) properties.fnShowError("In the add form is placed input element with the name '" + $(this).attr("name") + "' with the 'rel' attribute that must be less than a column count - " + iColumnCount, "add");
                else {
                    if (this.nodeName.toLowerCase() == "select" || this.tagName.toLowerCase() == "select") {
                        //sCellValue = $("option:selected", this).text();
                        sCellValue = $.map(
                        $.makeArray($("option:selected", this)), function(n, i) {
                            return $(n).text();
                        }).join(",");
                    } else if (this.nodeName.toLowerCase() == "span" || this.tagName.toLowerCase() == "span") sCellValue = $(this).html();
                    else {
                        if (this.type == "checkbox") {
                            if (this.checked) sCellValue = (this.value != "on") ? this.value : "true";
                            else sCellValue = (this.value != "on") ? "" : "false";
                        } else sCellValue = this.value;
                    }
                    //Deprecated
                    sCellValue = sCellValue.replace("DATAROWID", iDT_RowId);
                    sCellValue = sCellValue.replace(properties.sIDToken, iDT_RowId);

                    if (oSettings.aoColumns != null && oSettings.aoColumns[rel] != null && isNaN(parseInt(oSettings.aoColumns[0].mData))) {
            ...
            if (oSettings.aoColumns != null && isNaN(parseInt(oSettings.aoColumns[0].mData))) {
                return rowData;
            } else {
                return values;
            }
        ...
        function fnUpdateRowOnSuccess(nActionForm) {
            ...
            for (var rel = 0; rel < iColumnCount; rel++) {
                ...
                if (oSettings.aoColumns != null && oSettings.aoColumns[rel] != null && isNaN(parseInt(prop))) {
                    sCellValue = rowData[prop];
                } else {
        ...
        return this.each(function() {
            ...
            if (properties.bUseKeyTable) {
                ...
                keys.event.action(null, null, function(nCell) {
                    if ($(nCell).hasClass(properties.sReadOnlyCellClass)) return; /* Block KeyTable from performing any events while jEditable is in edit mode */
                    keys.block = true; /* Dispatch click event to go into edit mode - Saf 4 needs a timeout... */
                    setTimeout(function() {
                        $(nCell).dblclick();
                    }, 0);
