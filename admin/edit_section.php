...
<div id="editsection">
  <form id="formEditSection" action="?p=modules&id=<?php echo $_GET['id']; ?>&request=edit_section" method="post">
    ...
    <fieldset id="feedback">
      <div id="errors">
        <?php echo (isset($_POST['errors'])) ? $_POST['errors'] : ''; ?>
      </div>
    </fieldset>
    <input type="hidden" id="moduleId" value="<?php echo $moduleId; ?>" />
    <input type="hidden" id="order" value="<?php echo $order; ?>" />
    <button type="submit" id="submit">Create Section</button>
  </form>
</div>
...
<script type="text/javascript">
  //hide submit button
  $(function() {
    $('#formEditSection #submit').hide();
  });

  //validate form submission
  $('#formEditSection').submit(function() {
    var submit = false;
    var errors = '';

    if ($('#formEditSection #title').val().length == 0) {
      $('#formEditSection #title').css('border-color', 'orange').siblings('a').css('display','inline-block');
    ...
    if(submit) {
      $.ajax({
        ...
        data: {
          ajax        : true,
          submit      : true,
          title       : $('#formEditSection #title').val(),
          order       : $('#formEditSection #order').val()
        },
        dataType: "xml",
